...
from community_iot.models import HouseholdProfile, TelemetrySnapshot
...
class UnitRuntime:
    ...
    gas_incident_ticks: int = 0
    smoke_incident_ticks: int = 0
    overload_incident_ticks: int = 0
    water_incident_ticks: int = 0
    door_incident_ticks: int = 0
    air_incident_ticks: int = 0
...
class CommunitySimulator:
    ...
    def _meal_factor(self, hour: float) -> float:
        windows = (7.5, 12.0, 19.0)
        return max(math.exp(-((hour - center) ** 2) / 1.6) for center in windows)
    ...
    def _inject_periodic_challenge(self) -> None:
        # Keep the dashboard lively by injecting realistic random incidents periodically.
        ...
        if incident == "gas":
            runtime.gas_incident_ticks = max(runtime.gas_incident_ticks, self.rand.randint(4, 9))
        elif incident == "water":
            runtime.water_incident_ticks = max(runtime.water_incident_ticks, self.rand.randint(5, 12))
        elif incident == "overload":
            runtime.overload_incident_ticks = max(runtime.overload_incident_ticks, self.rand.randint(4, 8))
        elif incident == "smoke":
            runtime.smoke_incident_ticks = max(runtime.smoke_incident_ticks, self.rand.randint(4, 10))
    ...
    def _simulate_unit(self, profile: HouseholdProfile, runtime: UnitRuntime, hour: float) -> TelemetrySnapshot:
        ...
        if runtime.gas_incident_ticks == 0 and stove_on and self.rand.random() < 0.018 * profile.vulnerability:
            runtime.gas_incident_ticks = self.rand.randint(4, 10)
        if runtime.smoke_incident_ticks == 0 and stove_on and self.rand.random() < 0.04 * profile.vulnerability:
            runtime.smoke_incident_ticks = self.rand.randint(3, 8)
        if runtime.overload_incident_ticks == 0 and self.rand.random() < 0.012 * profile.vulnerability:
            runtime.overload_incident_ticks = self.rand.randint(4, 9)
        if runtime.water_incident_ticks == 0 and self.rand.random() < 0.006 * profile.vulnerability:
            runtime.water_incident_ticks = self.rand.randint(6, 15)
        if runtime.air_incident_ticks == 0 and self.rand.random() < 0.007 * profile.vulnerability:
            runtime.air_incident_ticks = self.rand.randint(8, 18)
        if runtime.door_incident_ticks == 0 and not occupancy and (hour >= 23 or hour < 5) and self.rand.random() < 0.008:
            runtime.door_incident_ticks = self.rand.randint(2, 6)

        gas_incident = runtime.gas_incident_ticks > 0
        smoke_incident = runtime.smoke_incident_ticks > 0
        overload_incident = runtime.overload_incident_ticks > 0
        water_incident = runtime.water_incident_ticks > 0
        air_incident = runtime.air_incident_ticks > 0
        door_incident = runtime.door_incident_ticks > 0
        ...
        gas_ppm = (
            4.0
            + (10.0 if stove_on else 0.0)
            + (self.rand.uniform(14.0, 36.0) if gas_incident else 0.0)
            + self.rand.uniform(0.0, 2.4)
        )
        smoke_ppm = (
            6.0
            + (18.0 if stove_on else 0.0)
            + (self.rand.uniform(24.0, 56.0) if smoke_incident else 0.0)
            + self.rand.uniform(0.0, 3.6)
        )
        pm25 = (
            22.0
        ...
        energy_increment = max(0.05, power_w / 1000 * (self.step_minutes / 60))
        ...
        runtime.energy_today_kwh += energy_increment
        ...
        for attr in (
            "gas_incident_ticks",
            "smoke_incident_ticks",
            "overload_incident_ticks",
            "water_incident_ticks",
            "door_incident_ticks",
            "air_incident_ticks",
        ):
    ...
    def step(self) -> tuple[list[TelemetrySnapshot], datetime]:
        ...
        for profile in self.profiles:
            runtime = self.runtime[profile.unit_id]
            snapshots.append(self._simulate_unit(profile, runtime, hour))
