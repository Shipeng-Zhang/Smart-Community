...
class SmartCityOAHTTPServer(ThreadingHTTPServer):
...
class SmartCityOAHandler(SimpleHTTPRequestHandler):
...
def run_server(
    *,
    ...
    frontend_dir = root / "frontend_oa"
    frontend_dir.mkdir(parents=True, exist_ok=True)
    SmartCityOAHandler.frontend_dir = frontend_dir

    hub = CommunityHub(household_count=household_count, seed=seed, tick_seconds=tick_seconds)
    hub.start()
    server = SmartCityOAHTTPServer((host, port), SmartCityOAHandler, hub=hub, frontend_dir=frontend_dir)
    print(f"Smart community OA dashboard running at http://127.0.0.1:{port}")
    try:
